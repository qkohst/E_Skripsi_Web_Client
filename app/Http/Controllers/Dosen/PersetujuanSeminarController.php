...
class PersetujuanSeminarController extends Controller
{
    ...
    {
        $this->_url = 'http://127.0.0.1:8000/api/v1/dosen/persetujuanseminar';
    ...
    public function index()
    {
        ...
        if ($response->status() == 200) {
            $persetujuanseminar = $response->json()['data'];
            return view('dosen.persetujuanseminar.index', compact('persetujuanseminar'));
        }
    ...
    public function update(Request $request, $id)
    {
        $response = Http::post($this->_url . '/' . $id, [
            ...
            'api_token' => session('api_token_user'),
            '_method'   => 'PATCH',
            'status_persetujuan_seminar' => $request->status_persetujuan_seminar,
            'catatan_persetujuan_seminar' => $request->catatan_persetujuan_seminar,
        ]);
        if ($response->status() == 200) {
